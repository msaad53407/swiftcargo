...
export const bulkUpdatePackageStatus = async (db, packageIds, newStatus, updatedBy) => {
  try {
    ...
    const updatePromises = validPackages.map(async (pkg) => {
      ...
      // Send notification
      await notifyPackageStatusUpdated(pkg.id, pkg.invoiceNo, newStatus, updatedBy.name || "");
    });
...
export function StatusUpdateModal({ isOpen, onClose, onConfirm, packageCount, isLoading }) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] p-4 sm:p-6">
        <DialogHeader>
          ...
          <DialogDescription>Update status for {packageCount} selected packages</DialogDescription>
        </DialogHeader>
...
export const bulkDuplicatePackages = async (db, packages, currentUser) => {
  try {
    ...
    const duplicatePromises = packages.map(async (originalPkg) => {
      ...
      await notifyPackageAdded(packageDoc.id, packageData.invoiceNo, currentUser?.name || "");
...
export function PendingAmountModal({ isOpen, onClose, onConfirm, packageCount, isLoading }) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] p-4 sm:p-6">
        <DialogHeader>
          ...
          <DialogDescription>Update pending amount for {packageCount} selected packages</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          <div className="space-y-2">
...
export function BulkActions({ selectedPackages, onBulkAction, currentUser, db, loadPackages, currentPage, packages }) {
  ...
  const handlePendingAmountUpdate = async (newAmount: string) => {
    setIsLoading(true);
    try {
      const success = await bulkUpdatePendingAmount(db, selectedPackages, newAmount, {
        name: currentUser?.name || "",
        email: currentUser?.email || "",
      });
  ...
  const handleStatusUpdate = async (newStatus: string) => {
    setIsLoading(true);
    try {
      const success = await bulkUpdatePackageStatus(db, selectedPackages, newStatus, {
        name: currentUser?.name || "",
        email: currentUser?.email || "",
      });
